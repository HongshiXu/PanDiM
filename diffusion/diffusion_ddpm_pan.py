...
from utils.loss_utils import GT_FDDL, Spatial_FDDL
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        ms_channels=8,
        # GT-FDDL parameters
        gt_fddl_high_boost=2.0,
        # Curriculum learning for Spatial_FDDL
        max_iterations=200000,
        prior_warmup_ratio=0.5,
    ):
        super().__init__()
        ...
        assert loss_type in ["l1", "l2", "l1ssim", "l1sam", "gtfddl"]
        # p2 loss weight
        self.p2_loss_weight_gamma = p2_loss_weight_gamma
        self.p2_loss_weight_k = p2_loss_weight_k

        # FDDL config
        self.use_fddl = use_fddl
        self.fddl_weight = fddl_weight
        self.ms_channels = ms_channels
        self.gt_fddl_high_boost = gt_fddl_high_boost
        # Curriculum learning config
        self.max_iterations = max_iterations
        self.prior_warmup_ratio = prior_warmup_ratio
    ...
    def set_loss(self, device):
        ...
        elif self.loss_type == "gtfddl":
            self.loss_func = GT_FDDL(
                high_boost=self.gt_fddl_high_boost, low_weight=1.0
            ).to(device)
        else:
            raise NotImplementedError()
        # Spatial_FDDL: 物理先验损失，配合余弦衰减课程学习（可选）
        if self.use_fddl:
            self.spatial_fddl = Spatial_FDDL(weight=1.0).to(device)

    def _get_prior_weight(self, current_iter):
        """余弦衰减课程学习：前 warmup_ratio 保持最大权重，后半段余弦衰减到 0"""
        if current_iter is None:
            return self.fddl_weight
        warmup_end = int(self.max_iterations * self.prior_warmup_ratio)
        if current_iter < warmup_end:
            return self.fddl_weight
        else:
            remaining = self.max_iterations - warmup_end
            if remaining <= 0:
                return 0.0
            progress = (current_iter - warmup_end) / remaining
            return self.fddl_weight * 0.5 * (1 + math.cos(math.pi * progress))
    ...
    def p_losses(self, x_start, noise=None, cond=None, current_iter=None, **kwargs):
        ...
        if self.self_condition and random.random() < 0.5:
            with torch.no_grad():
                if not self.conditional:
                    model_out = self.model(x_noisy, t, self_cond=None, **kwargs)
                else:
                    model_out = self.model(x_noisy, t, cond=cond, self_cond=None, **kwargs)
                if self.pred_mode == "noise":
        ...
        if not self.conditional:
            model_predict = self.model(x_noisy, t, self_cond=x_self_cond, **kwargs)
        else:
            model_predict = self.model(x_noisy, t, cond=cond, self_cond=x_self_cond, **kwargs)
        ...
        # Spatial_FDDL: 物理先验损失 + 余弦衰减课程学习
        # 仅在 x_start 预测模式下可用，且需要 cond 提供 LMS 和 PAN
        if self.use_fddl and self.pred_mode == "x_start" and cond is not None:
            ms_ch = self.ms_channels
            lms = cond[:, :ms_ch, :, :]          # (B, 8, H, W)
            pan = cond[:, ms_ch:ms_ch+1, :, :]   # (B, 1, H, W)
            pred_hrms = recon_x0 + lms            # 残差 → 实际 HRMS
            # 余弦衰减权重：前 warmup_ratio 保持 fddl_weight，后半段衰减到 0
            lambda_weight = self._get_prior_weight(current_iter)
            if lambda_weight > 1e-8:
                fddl_loss = self.spatial_fddl(pred_hrms, lms, pan)
                loss = loss + lambda_weight * fddl_loss
