...
class AnalysisMetrics:
    ...
    def batch_metrics(self, pred, gt, mode):
        for i in range(pred.shape[0]):
            # pred and gt are (C, H, W) tensors, float [0, 1]
            c = pred.shape[1]
            
            # Convert to numpy (H, W, C)
            pred_np = pred[i].detach().cpu().numpy().transpose(1, 2, 0)
            gt_np = gt[i].detach().cpu().numpy().transpose(1, 2, 0)
            
            # Depending on channel count, we might want to scale or strictly follow integer metrics.
            # However, for SAM/ERGAS in RS, usually float is fine.
            # But standard PSNR is often defined on 8-bit or dynamic range.
            
            # If using PIL-like behavior for 3/4 channels, we adhere to that.
            # For 8 channels, we keep float but assume dynamic range 1.0 or scale.
            
            # To be consistent with existing 'psnr' function default (255), 
            # we should either scale inputs to 0-255 or change dynamic_range to 1.
            
            # Let's use dynamic_range=1.0 for float inputs in 'psnr' if we don't scale.
            # But the 'psnr' function takes dynamic_range arg but ref_evaluate calls it with default 255.
            # So scaling to 255 is the safest minimal change to match current logic.
            ...
            gt_np = np.clip(gt_np, 0, 255)
            
            metrics = self.ref_metrics(pred_np, gt_np)
            for key in metrics:
